...
const filterDate = () => {
  ...
  if(rentalDate.value === '' && deliveryDate.value === '') return rentals.value = rentalStore.rentals

  if(rentalDate.value === '' && deliveryDate.value !== '') {
    rentals.value = rentalStore.rentals.filter((registro) => {
      return formatDate(deliveryDate.value) <= registro.deliveryDate;
    });
  }

  if(rentalDate.value !== '' && deliveryDate.value === '') {
    rentals.value = rentalStore.rentals.filter((registro) => {
      registro.rentalDate >= formatDate(rentalDate.value)
    });
  }
