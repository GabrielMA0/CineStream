...
const filterDate = (e: any) => {
  ...
  rentals.value = rentalStore.rentals.filter((registro) => {
    return registro.rentalDate >= formattedDate;
  });
}
</script>
  ...
  <div class="flex justify-between gap-2.5 w-full flex-wrap">
    ...
    <div class="flex gap-4 flex-wrap">
      ...
      <div class="flex gap-2.5 flex-wrap">
        <Input @change="filterDate" typeField="date" styleField="date" placeHolder="Filtrar data de locação"></Input>
        <Input typeField="date" styleField="date" placeHolder="Filtrar data de entrega"></Input>
        <Button>Filtrar datas</Button>
      </div>
    </div>
