...
const filterMoviesTitle = async () => {

    const result = await searchMovies(fieldMovie.value, fieldYear.value);

    movies.value = result.Search || [];
}

const filterMoviesYear = () => {
...
<template>
        ...
        <div class="flex gap-4 flex-wrap">
            <Input v-model='fieldMovie' styleField="filter" placeHolder="Buscar por título" @input="filterMoviesTitle"></Input>
            <Input v-model='fieldYear' typeField="number" styleField="filter" placeHolder="Buscar por Ano" @input="filterMoviesTitle"></Input>
        </div>
