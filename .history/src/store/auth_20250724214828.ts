    ...
    actions: {
        login(values: User): boolean {
            ...
            if (users && users.length > 0) {
                const found = users.find(u => u.document === values.document && u.password === values.password && u.status !== 'desativado');
                if (found) {
                    const userCopy = { ...found, token: fakeToken };
                    this.user = userCopy;
                    return true;
                }
                if (values.document === adminAccount.document && values.password === adminAccount.password) {
