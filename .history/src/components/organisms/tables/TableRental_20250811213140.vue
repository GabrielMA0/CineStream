...
<template>
  <Table>
    ...
    <tbody >
        <tr class="h-[72px]" v-for="rental in rentals" :key="rental.idRental">
            <Td class="capitalize">{{ rental.name }}</Td>
            <Td class="hidden md:table-cell">{{ rental.movie }}</Td>
            <Td>{{ rental.rentalDate }}</Td>
            <Td>{{ rental.deliveryDate }}</Td>
            <Td class="hidden md:table-cell capitalize">{{ rental.user }}</Td>
            <Td>
                <Status @click="toggleStatus(rental)" class="hover:bg-[#c57c00] cursor-pointer" :statusType="rental.status">{{ rental.status }}</Status>
            </Td>
        </tr>
