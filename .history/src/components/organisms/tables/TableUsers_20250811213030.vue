...
<template>
  <Table v-show="users.user.length > 0" class="w-full">
    <Thead>
        <Th>ID</Th>
        <Th>Nome</Th>
        <Th class="hidden md:table-cell">Documento</Th>
        <Th class="hidden md:table-cell">Senha</Th>
        <Th>Status</Th>
        <Th>Ações</Th>
    </Thead>

    <tbody>
      <tr class="h-[72px]" v-for="user in users.user" :key="user.id">
        <Td>{{ user.id }}</Td>
        <Td class="capitalize">{{ user.name }}</Td>
        <Td class="hidden md:table-cell">{{ user.document }}</Td>
        <Td class="hidden md:table-cell">{{ user.password }}</Td>
        <Td>
            <Status :statusType="user.status">{{ user.status }}</Status>
        </Td>
        <Td>
