...
const updateRegistration = handleSubmit((values: Clients) => {
  ...
  client.updateClient(id, clientData);
    ...
    <form @submit.prevent="onSubmit" class="flex flex-col justify-center items-center gap-13">
        <ContainerInputs>
          ...
          <div class="flex gap-5 flex-wrap justify-center w-full md:w-auto">
            <Button v-if="Object.keys(clientEdit).length === 0 && clientEdit.constructor === Object" typeBtn="button" @click="updateRegistration">Salvar</Button>
            <Button v-else typeBtn="submit">Cadastrar</Button>
            <Button v-show="Object.keys(clientEdit).length === 0 && clientEdit.constructor === Object" typeBtn="button" @click="changeClientStatus">Ativar/Desativar</Button>
        </div>
